    ...
    try:
        ...
        if voiding_log_channel:
            try:
                ...
                affected_users = [f'<@{g.get("discordid")}>: {g.get("result")}' for g in recent_games]
                embed = discord.Embed(
                    title=f"Game Voided: {gameid}",
                ...
                embed.add_field(name="Team 1", value=(', '.join([f'<@{pid}>' for pid in team1_ids]) or 'None'), inline=False)
                embed.add_field(name="Team 2", value=(', '.join([f'<@{pid}>' for pid in team2_ids]) or 'None'), inline=False)
                embed.add_field(name="Voided By", value=staff_mention, inline=True)
                embed.add_field(name="Affected Users", value='\n'.join(affected_users) or 'None', inline=False)
                await voiding_log_channel.send(embed=embed)
            except Exception as e:
        ...
        for recent_game in recent_games:
            ...
            if user:
                ...
                if websocket_enabled:
                    if recent_game.get('finalkills'):
                        finalkills -= int(recent_game.get('finalkills', 0))
                    if recent_game.get('diamonds'):
                        diamonds -= int(recent_game.get('diamonds', 0))
                    if recent_game.get('irons'):
                        irons -= int(recent_game.get('irons', 0))
                    if recent_game.get('gold'):
                        gold -= int(recent_game.get('gold', 0))
                    if recent_game.get('emeralds'):
                        emeralds -= int(recent_game.get('emeralds', 0))
                    if recent_game.get('blocksplaced'):
                        blocksplaced -= int(recent_game.get('blocksplaced', 0))
                ...
                print(f'Reverted elo and stats for user {player_id}: {new_elo}, wins: {wins}, losses: {losses}')
            ...
            print(f'Updated recentgames for player {player_id} in game {gameid} to voided.')
        ...
        print(f'Updated game state for gameid {gameid} to voided and cleared MVPs list.')
        ...
        if game_channels:
            ...
            try:
                ...
                try:
                    await asyncio.create_task(create_transcript(bot, int(text_channel_id), f"Game #{gameid} Transcript"))
                ...
                if text_channel_id:
                    try:
                        await asyncio.create_task(delete_channel(int(text_channel_id), guild))
