...
class StateSelect(discord.ui.Select):
    def __init__(self, parent_view, current_state):
        options = [
            discord.SelectOption(label="All", value="all", default=current_state == "all"),
            discord.SelectOption(label="Scored", value="scored", default=current_state == "scored"),
            discord.SelectOption(label="Voided", value="voided", default=current_state == "voided"),
            discord.SelectOption(label="Pending", value="pending", default=current_state == "pending"),
        ]
...
class IntervalSelect(discord.ui.Select):
    def __init__(self, parent_view, current_interval):
        options = [
            discord.SelectOption(label="1 Day", value="1", default=current_interval == 1),
            discord.SelectOption(label="5 Days", value="5", default=current_interval == 5),
            discord.SelectOption(label="10 Days", value="10", default=current_interval == 10),
            discord.SelectOption(label="15 Days", value="15", default=current_interval == 15),
            discord.SelectOption(label="30 Days", value="30", default=current_interval == 30),
        ]
...
class GamesCountView(discord.ui.View):
    ...
    async def update_graph(self, interaction: discord.Interaction):
        file, embed = make_games_graph(self.db_manager, self.state, self.interval, self.bot)
...
def make_games_graph(db_manager, state, interval, bot):
    ...
    date_buckets = [(start_date + timedelta(days=i)).date() for i in range(interval + 1)]
    ...
    ax.set_title('Games Count Over Time', fontsize=18, fontweight='bold', color='#43c6ea')
...
class AdminGamesCount(commands.Cog):
    ...
    @commands.command(name='gamescount', help='Show a graph of games count by state and interval.')
