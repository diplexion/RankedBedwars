...
class BanManager:
    ...
    async def ban_user(
        ...
        try:
            ...
            self.db.update_one(
                "users", {"discordid": str(discord_id)}, {"$set": update_data}
            )
            ...
            if channel:
                ban_expiry_time = datetime.fromtimestamp(ban_duration).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    async def unban_user(
        self,
        discord_id: str,
        unban_reason: str = "No reason provided",
        unbanned_by: str = "System",
    ) -> bool:
        try:
            user_data = self.db.find_one("users", {"discordid": str(discord_id)})
            if not user_data:
            ...
            if guild:
                member = await guild.fetch_member(int(discord_id))
                if member:
                    banned_role = guild.get_role(
                        int(self.bot.config["roles"]["rankedban"])
                    )
